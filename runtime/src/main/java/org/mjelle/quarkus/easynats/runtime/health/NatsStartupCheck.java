 ...
 *
 * <p>The startup probe indicates whether the application has completed its initialization. Once the
 * NATS connection is established for the first time, this probe will report "UP" and will continue
 * to do so for the life of the application. This "latching" behavior prevents the application from
 * being terminated if the NATS connection is lost after a successful startup.
 *
 * <p>It returns: - UP if the NATS connection has been established at least once. - DOWN if the NATS
...
public class NatsStartupCheck extends AbstractNatsReadinessProbe {
  ...
  public HealthCheckResponse call() {
    ...
    // If latched, always report UP, but preserve the underlying connection status data for
    // observability.
    if (isReady.get()) {
