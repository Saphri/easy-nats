 ...
 * <p>
 * This producer creates {@link io.nats.client.Options} instances from {@link NatsConfiguration}.
 * It uses {@link DefaultBean} annotation to allow developers to override the default Options
 * by providing their own unqualified {@link Options} bean in their application.
 * <p>
...
public class NatsConnectionProducer {
     ...
     * <p>
     * These dependencies are needed to build a complete, ready-to-use Options instance:
     * - TlsConfigurationRegistry: Resolves TLS configurations
     * - ExecutorService: Used for async operations in NATS connection
     * - ConnectionStatusHolder: Used for health check connection listener
     *
     ...
     * <p>
     * This method is annotated with {@link DefaultBean}, which means:
     * - If no other unqualified {@link Options} bean exists, this producer creates the default
     * - If a developer provides a custom {@link Options} bean, this producer is bypassed completely
     * - CDI bean resolution handles the selection automatically
     * <p>
     * The produced Options includes:
     * - Server URLs from {@code quarkus.easynats.servers}
     * - Authentication (if configured) from {@code quarkus.easynats.username} and {@code quarkus.easynats.password}
     * - SSL/TLS settings from {@code quarkus.easynats.ssl-enabled} and {@code quarkus.easynats.tls-configuration-name}
     *
     ...
     * <p>
     * Validation rules:
     * - {@code quarkus.easynats.servers} must be present and non-empty
     * - {@code quarkus.easynats.username} and {@code quarkus.easynats.password} must be present together
     * <p>
     * If validation fails, a {@link NatsConfigurationException} is thrown with a clear error message.
     *
