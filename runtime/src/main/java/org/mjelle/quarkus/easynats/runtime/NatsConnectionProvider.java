...
/**
 * Provider that creates and manages the lifecycle of NatsConnection instances. This is a singleton
 * CDI bean that creates the NATS connection based on configuration.
 *
 * <p>The provider validates configuration at runtime and creates a wrapped connection that
 * delegates to the underlying NATS connection but provides a safe close() method.
 */
@ApplicationScoped
public class NatsConnectionProvider {
   ...
   *
   * <p>The Options are injected from the CDI container, which provides either: - Default Options
   * from NatsConnectionProducer (if no custom bean exists) - Custom Options from a
   * developer-provided bean (if @Produces @Unremovable bean exists)
   *
   * <p>The Options object is fully configured and includes: - Server URLs and authentication (from
   * NatsConnectionProducer) - Executor service for async operations (added by producer) -
   * Connection listener for health checks (added by producer) - SSL/TLS configuration (added by
   * producer)
   *
   * <p>This follows the CDI @DefaultBean pattern: developers can completely override the default
   * Options by providing their own unqualified Options bean.
   *
   ...
   */
  public NatsConnectionProvider(Options options, ConnectionStatusHolder statusHolder) {
   ...
   *
   * <p>The connection is created during application startup via onStartup() method. This producer
   * simply returns the pre-created connection.
   *
  ...
  /**
   * Ensures the NATS connection is created early during application startup. Runs first (priority
   * 1) to ensure connection is ready for all other startup tasks.
   *
   * @param startupEvent the startup event
   */
  void onStartup(@Observes @Priority(1) StartupEvent startupEvent) {
    log.info(
        "NatsConnectionProvider: Initializing connection on startup (priority 1 - runs first)");
    try {
      produceConnection();
   ...
   *
   * <p>This method uses fully-configured Options (injected via CDI) to establish the connection.
   * The Options are created by either: - Default NatsConnectionProducer (from configuration
   * properties and runtime settings) - Custom developer-provided bean (if present, overrides
   * default)
   *
   * <p>The connection is wrapped in a NatsConnection facade that prevents accidental closure.
   *
   * @return a wrapped NATS connection
    ...
    try {
      ...
      if (natsConnection == null) {
        throw new NatsConfigurationException(
            "Failed to create NATS connection: connection is null");
      }
      ...
      log.infof(
          "Successfully established NATS connection to: %s", natsConnection.getConnectedUrl());
      ...
      throw new RuntimeException(
          "Failed to connect to NATS broker at startup. "
              ...
              + "Ensure NATS server is running and Options configuration is correct.",
          e);
    }
  }

  /**
   * Returns the wrapped connection if already created, or null if not initialized. This is for
