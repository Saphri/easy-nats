 ...
 *
 * <p>The liveness probe indicates whether the application is running. It returns: - UP if the NATS
 * connection is CONNECTED, RECONNECTING, RECONNECTED, RESUBSCRIBED, or in LAME_DUCK state - DOWN
 * only if the connection is CLOSED (permanently)
 *
 * <p>The liveness probe does not report DOWN during temporary disconnections (DISCONNECTED state)
...
public class NatsHealthCheck implements HealthCheck {
  ...
  public HealthCheckResponse call() {
    ...
    if (status == ConnectionStatus.CLOSED) {
      return HealthCheckResponse.named("NATS Connection (Liveness)")
          .down()
