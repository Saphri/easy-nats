 ...
 *
 * <p>Ensures that only Jackson-compatible types (POJOs with no-arg constructors, records, types
 * with Jackson annotations) are used. Rejects primitives, arrays, and types that cannot be
 * instantiated by Jackson.
 */
public class TypeValidator {
  ...
  public TypeValidationResult validate(Class<?> type) {
    if (type == null) {
      return new TypeValidationResult(
          false, "null", "Type cannot be null", ValidationErrorType.CUSTOM_ERROR);
    }
  ...
  private TypeValidationResult createMissingNoArgCtorError(Class<?> type) {
    ...
    return new TypeValidationResult(
        false, type.getName(), message, ValidationErrorType.MISSING_NO_ARG_CTOR);
  }
