...
/**
 * Service for managing distributed tracing spans across NATS messaging operations. Integrates with
 * OpenTelemetry to create producer and consumer spans and propagate W3C Trace Context headers
...
public class NatsTraceService {
  ...
  /**
   * Reusable TextMapGetter for extracting trace context from NATS Headers. This is created once and
   * reused for all trace context extraction operations.
   */
  private static final TextMapGetter<Headers> HEADERS_GETTER =
   ...
   *
   * <p>OpenTelemetry is provided by Quarkus when the quarkus-opentelemetry extension is present. If
   * OpenTelemetry is not configured or no exporter is available, tracing will be disabled.
  ...
  public NatsTraceService(OpenTelemetry openTelemetry) {
    ...
    if (openTelemetry != null && openTelemetry != OpenTelemetry.noop()) {
      this.tracer = openTelemetry.getTracer(NatsTraceService.class.getCanonicalName(), "1.0");
      this.tracingEnabled = true;
      LOGGER.infof(
          "OpenTelemetry tracing initialized successfully: %s",
  ...
  /**
   * Creates a producer span for publishing a message and injects W3C Trace Context headers into the
   * provided headers map.
   *
   * <p>The producer span represents the act of publishing a message to a NATS subject.
   *
  ...
  public Span createProducerSpan(String subject, Headers headers) {
    ...
    Span span =
        tracer
            .spanBuilder(NatsConstants.SPAN_NAME_PUBLISH_PREFIX + subject)
  ...
  /**
   * Creates a consumer span for receiving a message from a NATS subject. Extracts W3C Trace Context
   * from the message headers to link with the producer span.
   *
   * <p>The consumer span represents the act of receiving and processing a message from a NATS
   * subject.
   *
  ...
  public Span createConsumerSpan(String subject, Message message) {
    ...
    Span span =
        tracer
            .spanBuilder(NatsConstants.SPAN_NAME_RECEIVE_PREFIX + subject)
  ...
  /**
   * Injects the current W3C Trace Context into NATS message headers. This ensures that distributed
   * traces are properly linked across services.
   *
  ...
  /**
   * Extracts the W3C Trace Context from NATS message headers and returns the corresponding
   * OpenTelemetry Context.
   *
   * <p>If no trace context is found, returns the current context.
   *
  ...
  private Context extractTraceContext(Message message) {
    ...
    try {
      ...
      Context extractedContext =
          propagator.extract(Context.current(), message.getHeaders(), HEADERS_GETTER);
  ...
  /**
   * Creates a scope for executing code within a span's context. Used to activate a span so that
  ...
  /**
   * Marks a span as failed with the given exception. Sets the span status to ERROR and records the
   * exception event.
   *
  ...
  public void recordException(Span span, Throwable exception) {
    try {
      span.setStatus(
          StatusCode.ERROR, exception != null ? exception.getMessage() : "Unknown error");
      if (exception != null) {
