 ...
 *
 * <p>
 * CloudEvents binary-mode format stores:
 * - Attributes (ce-specversion, ce-type, ce-source, ce-id, etc.) in NATS message headers with "ce-" prefix
 * - Event data in NATS message payload
 * </p>
 *
 * <p>
 * This class validates that a NATS message is a valid CloudEvents 1.0 binary-mode message
 * and extracts the event data for further processing.
 * </p>
 */
public class CloudEventUnwrapper {
     ...
     *
     * <p>
     * Validates that the message contains all required CloudEvents 1.0 binary-mode headers
    ...
    public static byte[] unwrapData(Message message) throws CloudEventException {
        ...
        if (type == null) {
            throw new CloudEventException(
                    "Missing required CloudEvents header: " + CE_TYPE_HEADER);
        }

        String source = getHeaderValue(message, CE_SOURCE_HEADER);
        if (source == null) {
            throw new CloudEventException(
                    "Missing required CloudEvents header: " + CE_SOURCE_HEADER);
        }

        String id = getHeaderValue(message, CE_ID_HEADER);
        if (id == null) {
            throw new CloudEventException(
                    "Missing required CloudEvents header: " + CE_ID_HEADER);
        }

        // Validate spec version
        if (!SPEC_VERSION_1_0.equals(specVersion)) {
            throw new CloudEventException(
                    "Unsupported CloudEvents version: " + specVersion
                            + " (expected 1.0)");
        }
