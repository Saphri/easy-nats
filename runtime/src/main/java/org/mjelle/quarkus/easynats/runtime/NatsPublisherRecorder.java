...
public class NatsPublisherRecorder {
   ...
   *
   * <p>This method is called by CDI to produce instances of {@link NatsPublisher}. It uses the
   * {@link NatsSubject} annotation to configure the default subject for the publisher.
   *
   * <p>NatsTraceService is injected by CDI at runtime. OpenTelemetry will be injected automatically
   * by Quarkus when the quarkus-opentelemetry extension is present.
   *
   * <p>The global {@link Codec} bean is also injected and used for serialization of message
   * payloads.
   *
   * @param injectionPoint the injection point
   * @param connectionManager the NATS connection manager
   * @param codec the global payload codec (injected by CDI)
   * @param traceService the NATS tracing service (may be a no-op implementation if tracing is
   *     disabled)
