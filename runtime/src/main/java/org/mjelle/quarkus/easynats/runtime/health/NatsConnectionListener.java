 ...
 *
 * <p>This listener is registered with the NATS connection and updates the ConnectionStatusHolder
 * whenever the connection status changes. This enables health checks to report accurate connection
 * status in real-time.
 */
public class NatsConnectionListener implements ConnectionListener {
  ...
  public void connectionEvent(Connection conn, Events event) {
    ...
    // Only update status if event maps to a state change (DISCOVERED_SERVERS returns null for
    // no-op)
    if (newStatus != null) {
