 ...
 *
 * <p>This class encapsulates the common logic for checks that report UP only when the NATS
 * connection is in a fully connected state (CONNECTED, RECONNECTED, RESUBSCRIBED). All other states
 * (including temporary disconnections and graceful shutdown) report DOWN.
 *
 * <p>Concrete implementations (e.g., readiness and startup probes) inherit this logic and only need
 * to provide their specific health check name.
 */
public abstract class AbstractNatsReadinessProbe implements HealthCheck {

  private static final Set<ConnectionStatus> HEALTHY_STATES =
      Set.of(
          ConnectionStatus.CONNECTED, ConnectionStatus.RECONNECTED, ConnectionStatus.RESUBSCRIBED);
