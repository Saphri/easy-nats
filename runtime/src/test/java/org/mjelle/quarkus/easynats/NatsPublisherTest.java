 ...
 *
 * <p>Tests cover: - POJO serialization and CloudEvents wrapping - Null payload rejection - Default
 * subject handling - CloudEvents header verification
 */
...
class NatsPublisherTest {
  ...
  void testPublishPojoAsCloudEvent() throws Exception {
    ...
    assertThat(headers.get("ce-type"))
        .hasSize(1)
        .contains("org.mjelle.quarkus.easynats.NatsPublisherTest.TestPojo");
  ...
  void testPublishComplexPojo() throws Exception {
    // Given
    ComplexTestPojo complex = new ComplexTestPojo("complex-id", new TestPojo("nested", 42), 42.5);
    ...
    ComplexTestPojo deserialized =
        objectMapper.readValue(payloadCaptor.getValue(), ComplexTestPojo.class);
