 ...
 *
 * <p>Tests the core framework logic that detects explicit vs implicit acknowledgment mode based on
 * subscriber method parameter types, and validates codec deserialization error handling behavior.
 */
...
class DefaultMessageHandlerTest {
  ...
  @Mock(lenient = true)
  private SubscriberMetadata metadata;
  ...
  void testIsNatsMessageTypeWithGenericType() throws Exception {
    ...
    Method method =
        TestSubscriber.class.getMethod(
            "handleWithExplicitMode", org.mjelle.quarkus.easynats.NatsMessage.class);
  ...
  void testExplicitModeCodecDecodeErrorAutoNak() throws Exception {
    ...
    // Then: Message IS auto-NAK'd (same behavior as implicit mode, since subscriber is never
    // invoked)
    verify(message).nak();
  ...
  private boolean isNatsMessageType(Type type) {
    if (type instanceof ParameterizedType) {
      ...
      return rawType instanceof Class
          && org.mjelle.quarkus.easynats.NatsMessage.class.isAssignableFrom((Class<?>) rawType);
