 ...
 *
 * <p>Tests the @DefaultBean CDI producer that creates Options from NatsConfiguration properties.
 * Validates that: - Options are created correctly from valid configuration - Required properties
 * are validated - Custom Options beans can override the default producer
 */
...
class NatsConnectionProducerTest {
  ...
  /**
   * Creates a mock TlsConfigurationRegistry that always returns empty (no TLS config). This is
   * sufficient for tests that don't test TLS configuration.
   */
  ...
  void testProducerThrowsExceptionWhenServerUrlIsNull() {
    ...
    NatsConfiguration config =
        new TestNatsConfiguration(Optional.of(servers), Optional.empty(), Optional.empty(), false);
  ...
  void testProducerErrorMessageIsClearForMissingServers() {
    ...
    NatsConfiguration config =
        new TestNatsConfiguration(
            Optional.empty(), Optional.of("user"), Optional.of("pass"), false);

    // When & Then
    assertThatThrownBy(() -> producer.natsOptions(config))
        .isInstanceOf(NatsConfigurationException.class)
        .hasMessageContainingAll(
            "quarkus.easynats.servers", "required", "application.properties", "custom");
  }
  ...
  void testProducerErrorMessageIsClearForAuthMismatch() {
    ...
    assertThatThrownBy(() -> producer.natsOptions(config))
        .isInstanceOf(NatsConfigurationException.class)
        .hasMessageContainingAll("username", "password", "both");
  }
}
