 ...
 *
 * <p>Tests cover: - Primitive type error messages include wrapper pattern examples - Array type
 * error messages include wrapper pattern examples - Missing no-arg constructor error messages
 * provide clear guidance - All error messages are properly formatted and actionable
 */
...
class ErrorMessageFormatterTest {
  ...
  void testDeserializationErrorIncludesContext() {
    ...
    // When
    String errorMsg =
        ErrorMessageFormatter.formatDeserializationError(typeName, rawPayload, rootCause);
