 ...
 *
 * This model demonstrates that the library passes annotated types to Jackson
 * unchanged. Jackson handles all annotation processing - the library just
 * delegates to ObjectMapper.writeValueAsBytes() and ObjectMapper.readValue().
 *
 * Annotations used:
 * - @JsonProperty("order_id"): Renames field 'id' to 'order_id' in JSON
 * - @JsonIgnore on internalId: Field is excluded from JSON serialization
 * - Regular fields without annotations: Normal JSON mapping
 */
public class AnnotatedOrderData implements Serializable {
    private static final long serialVersionUID = 1L;

    /**
     * Order ID field - mapped to "order_id" in JSON.
     * When serialized, this field appears as "order_id" in the JSON.
     * When deserialized, the "order_id" JSON key populates this field.
     */
    @JsonProperty("order_id")
    private String id;

    /**
     * Customer ID - normal field, no annotation.
     * Appears as "customerId" in JSON (standard Java naming convention).
     */
    private String customerId;

    /**
     * Total price - normal field, no annotation.
     * Appears as "totalPrice" in JSON (standard Java naming convention).
     */
    private double totalPrice;

    /**
     * Internal ID - excluded from JSON.
     * This field is NOT included when serializing to JSON.
     * When deserializing, if "internalId" appears in JSON, it's ignored.
     * This field will have its default value (null) after deserialization.
     */
    @JsonIgnore
