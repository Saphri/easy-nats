 ...
 * 1. The NATS Dev Services processor automatically starts a NATS container when needed
 * 2. The application can connect to the automatically started server
 * 3. Dev Services can be disabled via configuration
 * 4. Explicitly configured NATS servers prevent Dev Services from starting
 *
 * Uses default configuration from application.properties (NATS servers commented out),
 * allowing Dev Services to automatically start a NATS container.
 */
@QuarkusTest
...
class NatsDevServicesTest {
    ...
    @Test
    @DisplayName("Application connects successfully to Dev Services NATS server without explicit configuration")
