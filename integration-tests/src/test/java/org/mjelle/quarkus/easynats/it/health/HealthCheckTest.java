 ...
 *
 * <p>This test verifies that the Quarkus health check endpoints correctly report the status of the
 * NATS connection according to the specification.
 */
...
class HealthCheckTest {
   ...
   *
   * <p>Corresponds to spec acceptance scenario: Liveness User Story 1.1
   */
  @Test
   ...
   *
   * <p>Corresponds to spec acceptance scenario: Readiness User Story 2.2
   */
  @Test
   ...
   *
   * <p>Corresponds to spec acceptance scenario: Startup User Story 3.2
   */
  @Test
   ...
   *
   * <p>Corresponds to spec requirement: FR-010
   */
  @Test
  ...
  void testHealthResponseIncludesConnectionStatus() {
    ...
    assertThat(response.jsonPath().getList("checks")).isNotEmpty();
    assertThat(response.jsonPath().getString("checks[0].name"))
        .contains("NATS Connection")
        .contains("Liveness");
   ...
   *
   * <p>Corresponds to spec success criteria: SC-001, SC-002 (100ms response time)
   */
  @Test
   ...
   *
   * <p>Corresponds to spec requirement: FR-012 (Readiness behavior on disconnect) Note: This test
   * simulates the expected behavior when connection state changes. In a real scenario, the
   * ConnectionListener would update the status.
   */
  @Test
   ...
   *
   * <p>Corresponds to spec requirement: FR-011 (Liveness stays UP during reconnect) This verifies
   * that the liveness probe logic correctly distinguishes between temporary (RECONNECTING) and
   * permanent (CLOSED) disconnections.
   */
  @Test
   ...
   *
   * <p>Corresponds to spec requirement: FR-012 (Readiness DOWN on disconnect) This verifies that
   * readiness reports DOWN on DISCONNECTED state, while liveness would stay UP (if only
   * RECONNECTING).
   */
  @Test
   ...
   *
   * <p>Corresponds to spec requirement: FR-008, FR-009 (Startup probe behavior) This verifies that
   * startup probe reports consistent results with readiness.
   */
  @Test
  ...
  void testStartupProbeConsistency() {
    ...
    String readinessConnectionStatus =
        readinessResponse.jsonPath().getString("checks[0].data.connectionStatus");
    String startupConnectionStatus =
        startupResponse.jsonPath().getString("checks[0].data.connectionStatus");
