 ...
 *
 * <p>These tests ensure that the application fails fast and with clear error messages if any
...
class StartupValidationTest {
   ...
   *
   * <p>If this test class is instantiated and the app starts, then startup validation passed.
   */
  @Test
  ...
  /**
   * Note: Tests for startup failures (invalid consumer creation, connection failures, etc.) require
   * special test infrastructure to validate application startup failures. These are documented in
   * tasks.md as T018 but require test archives with expected startup exceptions.
   *
   * <p>For now, this test just verifies that valid configurations allow the app to start.
   */
}
