...
public class NatsTestUtils {
  ...
  public static Connection getConnection() throws Exception {
    if (connectionRef.get() == null
        || connectionRef.get().getStatus() != Connection.Status.CONNECTED) {
      ...
      Options options = new Options.Builder().server(natsUrl).userInfo(username, password).build();
  ...
  /**
   * Returns a NatsConnection wrapper for testing try-with-resources and lifecycle scenarios. This
   * method wraps the underlying connection in a NatsConnection facade.
   *
