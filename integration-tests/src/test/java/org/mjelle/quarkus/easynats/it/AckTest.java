 ...
 *
 * <p>These tests verify that: 1. Subscribers can receive NatsMessage&lt;T&gt; parameter (explicit
 * mode) 2. Subscribers can call ack() without error 3. The framework skips auto-ack when explicit
 * mode is used 4. Payload is properly deserialized and accessible
 */
@QuarkusTest
...
class AckTest {
  ...
  void testSubscriberReceivesNatsMessageAndCanCallAck() {
    ...
    given().when().get("/ack/error").then().statusCode(404); // 404 means no error was stored
  }
  ...
  void testPayloadIsDeserializedCorrectly() {
    ...
    OrderData[] orders = {
      new OrderData("ORD-002", "CUST-002", 250.00), new OrderData("ORD-003", "CUST-003", 350.00),
    };
  ...
  void testNatsMessageAccessorMethods() {
    ...
    given().when().get("/ack/error").then().statusCode(404);
  }
}
