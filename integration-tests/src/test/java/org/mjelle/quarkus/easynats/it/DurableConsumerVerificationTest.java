 ...
 *
 * <p>Tests that verify: - Application starts successfully when durable consumer exists on NATS
 * server - Durable consumer binding works correctly
 */
@QuarkusTest
...
class DurableConsumerVerificationTest {
   ...
   *
   * <p>This is the critical test for the durable consumer feature. If this test passes, it means:
   * 1. NatsStreamTestResource pre-creates a stream with a durable consumer named "test-consumer" 2.
   * DurableSubscriber annotation references stream="test-stream" and consumer="test-consumer" 3.
   * SubscriberInitializer calls jsm.getConsumerInfo() to verify the consumer exists 4. Application
   * startup succeeds because the consumer verification passes
   *
   * <p>If the durable consumer didn't exist, the application would fail to start with an
   ...
   *
   * <p>This subscriber is initialized during application startup. If the durable consumer doesn't
   * exist on the NATS server, the application will fail to start with a clear error: "Failed to
   * verify durable consumer: Stream 'test-stream' does not contain consumer 'test-consumer'"
   */
  @ApplicationScoped
