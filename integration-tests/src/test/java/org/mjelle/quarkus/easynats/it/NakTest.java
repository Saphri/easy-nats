 ...
 * 1. Subscribers can receive NatsMessage&lt;T&gt; parameter and call nak()
 * 2. Metadata provides redelivery count information
 * 3. Framework skips auto-ack/nak when explicit mode is used
 * 4. Payload is properly deserialized and accessible
 * 5. Subscribers can distinguish between first attempt and redelivery attempts
 * </p>
 */
@QuarkusTest
...
class NakTest {
    ...
    void testSubscriberCanCallNakWithoutError() {
        ...
        // Then: Verify the subscriber processed the message (called nak() on first attempt, then ack() on second)
        await()
                .atMost(Duration.ofSeconds(5))
                .pollInterval(Duration.ofMillis(100))
                .untilAsserted(() -> {
