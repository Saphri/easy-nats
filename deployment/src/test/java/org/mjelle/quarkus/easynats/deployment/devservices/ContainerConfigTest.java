...
/**
 * Unit tests for ContainerConfig record. Tests URL generation, configuration mapping, and
 * validation.
 */
...
class ContainerConfigTest {
  ...
  void testToConnectionUrlSingleContainer() {
    ContainerConfig config =
        new ContainerConfig("abc123def456", "localhost", "4222", "nats", "nats", false);
  ...
  void testToConnectionUrlWithTls() {
    ContainerConfig config =
        new ContainerConfig(
            "abc123def456", "localhost", "4222", "secure-user", "secure-pass", true);
  ...
  void testToConfigurationMap() {
    ContainerConfig config =
        new ContainerConfig("abc123def456", "localhost", "4222", "admin", "secretpass", false);

    Map<String, String> map = config.toConfigurationMap();

    assertThat(map)
        .containsEntry("quarkus.easynats.servers", "nats://localhost:4222")
  ...
  void testToConfigurationMapWithTls() {
    ContainerConfig config =
        new ContainerConfig("abc123def456", "localhost", "4222", "user", "pass", true);
  ...
  void testValidationHostEmpty() {
    assertThatThrownBy(() -> new ContainerConfig("abc123", "", "4222", "nats", "nats", false))
  ...
  void testValidationPortNonNumeric() {
    assertThatThrownBy(
            () -> new ContainerConfig("abc123", "localhost", "invalid", "nats", "nats", false))
        .isInstanceOf(IllegalArgumentException.class)
  ...
  void testValidationPortRange() {
    assertThatThrownBy(
            () -> new ContainerConfig("abc123", "localhost", "70000", "nats", "nats", false))
        .isInstanceOf(IllegalArgumentException.class)
  ...
  void testValidationUsernameEmpty() {
    assertThatThrownBy(() -> new ContainerConfig("abc123", "localhost", "4222", "", "nats", false))
        .isInstanceOf(IllegalArgumentException.class)
  ...
  void testValidationPasswordNull() {
    assertThatThrownBy(
            () -> new ContainerConfig("abc123", "localhost", "4222", "nats", null, false))
        .isInstanceOf(IllegalArgumentException.class)
  ...
  void testValidationContainerIdEmpty() {
    assertThatThrownBy(() -> new ContainerConfig("", "localhost", "4222", "nats", "nats", false))
        .isInstanceOf(IllegalArgumentException.class)
