              ...
              t -> {
                if (!(t instanceof DefinitionException)) {
                  throw new AssertionError(
                      "Expected DefinitionException but got: " + t.getClass().getName(), t);
                }
                String msg = t.getMessage();
                if (!msg.contains("must specify either")
                    || !msg.contains("subject")
                    || !msg.contains("stream")
                    || !msg.contains("consumer")) {
                  throw new AssertionError(
                      "Expected error message about specifying subject or stream/consumer but got: "
                          + msg,
                      t);
                }
              });
