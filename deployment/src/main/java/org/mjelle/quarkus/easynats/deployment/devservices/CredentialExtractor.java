 ...
 *
 * <p>This internal utility class encapsulates the logic for reading NATS configuration from
 * docker-compose container environment variables and applying sensible defaults.
 *
 * <h3>Environment Variables Supported</h3>
 *
 * <ul>
 *   <li>{@code NATS_USERNAME}: Standard NATS username (preferred)
 *   <li>{@code NATS_USER}: Alternative username variable (fallback for compatibility)
 *   <li>{@code NATS_PASSWORD}: NATS password (required for auth)
 *   <li>{@code NATS_TLS_CERT}: TLS certificate file path (indicates TLS enabled)
 *   <li>{@code NATS_TLS_KEY}: TLS key file path (indicates TLS enabled)
 *   <li>{@code NATS_TLS_CA}: TLS CA file path (indicates TLS enabled)
 * </ul>
 *
 ...
 *
 * <ul>
 *   <li>Username defaults to "nats"
 *   <li>Password defaults to "nats"
 *   <li>SSL/TLS defaults to disabled (false)
 * </ul>
 *
 ...
 *
 * <ul>
 *   <li>{@code NATS_TLS_CERT} environment variable is set
 *   <li>{@code NATS_TLS_KEY} environment variable is set
 *   <li>{@code NATS_TLS_CA} environment variable is set
 * </ul>
 *
 * This approach aligns with NATS server requirements which mandate certificate paths for TLS
 * enablement.
 *
 * <h3>Usage Example</h3>
 *
 ...
 * <h3>Design Notes</h3>
 *
 * - This is an internal (package-private) utility—not part of the public API - Static utility
 * methods avoid unnecessary instantiation - Immutable {@code Credentials} record ensures
 * thread-safety - No mutable state or side effects
 *
...
class CredentialExtractor {
  ...
  private static final String USERNAME_ALT_ENV_VAR =
      "NATS_USER"; // Fallback for backward compatibility
  ...
  public static Credentials extract(Map<String, String> containerEnv) {
    ...
    String username =
        containerEnv.getOrDefault(
            USERNAME_ENV_VAR, containerEnv.getOrDefault(USERNAME_ALT_ENV_VAR, DEFAULT_USERNAME));
    ...
    boolean ssl =
        containerEnv.containsKey(TLS_CERT_ENV_VAR)
