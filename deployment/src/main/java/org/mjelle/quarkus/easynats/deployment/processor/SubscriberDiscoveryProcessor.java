 ...
 *
 * <p>This processor scans the application classpath for {@code @NatsSubscriber} annotations and
...
public class SubscriberDiscoveryProcessor {
   ...
   *
   * <p>Validates that: 1. Method has exactly 1 parameter 2. Parameter type is Jackson-compatible
   * (not primitive, not array, has no-arg constructor)
   *
   ...
   *
   * <p>Rejects: - Primitive types (int, long, double, etc.) - Array types (int[], String[], etc.)
   * For complex types that might not have no-arg constructors, validation will happen at runtime
  ...
  private void validateJacksonCompatibility(
      Type parameterType, DotName paramTypeName, MethodInfo method, ClassInfo declaringClass) {
  ...
  private String getAnnotationValue(AnnotationInstance annotation, String propertyName) {
    try {
      ...
      LOGGER.warnf(
          e,
          "Could not extract annotation property '%s'. Defaulting to an empty string.",
          propertyName);
      return "";
    }
  }
   ...
   *
   * <p>Rules: - Either `subject` (ephemeral) OR `stream+consumer` (durable) must be provided -
   * Cannot have both `subject` and `stream/consumer` - If `stream` is provided, `consumer` must
   * also be provided (and vice versa) - All three cannot be empty
   *
  ...
  private void validateAnnotationProperties(
      String subject, String stream, String consumer, ClassInfo declaringClass, MethodInfo method) {
