 ...
 *
 * This immutable record encapsulates all connection metadata needed to establish
 * a NATS client connection. It is extracted from running docker-compose NATS containers
 * via environment variable inspection and Docker API access.
 *
 ...
 * - {@code host}: Non-empty, valid hostname or IP address
 * - {@code port}: Each port value must be numeric and in range 1-65535
 * - {@code username}: Non-empty string (default "nats")
 * - {@code password}: Non-null (empty string allowed per NATS spec)
 * - {@code containerId}: Non-empty Docker container identifier(s)
 * - {@code sslEnabled}: Boolean flag for TLS enablement
 *
 * <h3>Immutability</h3>
 * All fields are immutable. Use constructor for creation. The record pattern
 * ensures thread-safety and prevents accidental modification.
 *
 * <h3>Integration with Quarkus</h3>
 * The {@link #toConfigurationMap()} method generates Quarkus dev services
 * configuration that is automatically applied at build time.
 *
...
public record ContainerConfig(
     ...
     * For clustering: comma-separated list of all discovered ports
     */
    String port,
